= Prototipo de Arquetipo con Axon
:linkattrs:
:icons: font
...
TODO: copiado de version que usa spring cloud stream. Adaptar

== Introducción

Este proyecto es un prototipo que muestra una aplicación Java que usa Axon Framework para aplicar DDD sin *Event Sourcing* persitiendo los agregados a través de JPA. También aplica los principios
de Clean Architecture y utiliza Spring Cloud Stream para la integración basada en eventos con Kafka.

El prototipo está pensado para la creación de microservicios que gestionen entidades de negocio a través de eventos, implementando un enfoque CQRS básico y coreografía de Sagas entre módulos. Dentro
de este enfoque se busca mantener el mismo sistema de persistencia que las aplicaciones tradicionales basadas en CRUD de modo que puedan evolucionar manteniendo el mismo modelo de datos relacional.

Se ha utilizado un enfoque de arquitectura en "vertical slices" donde cada módulo contiene sus propias capas de dominio, aplicación, infraestructura e interfaces. Esto facilita la separación de responsabilidades y la escalabilidad del proyecto.
